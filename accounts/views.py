...
def registerVendor(request):
    if request.user.is_authenticated: #check if user is already logged in
        ...
        if form.is_valid() and v_form.is_valid(): # here we check if data from user, vendor form are valid
            ...
            mail_subject = "Please activate your account"
            email_template = "accounts/email/account_verification.html"
            send_verification_email(request, user, mail_subject, email_template) # sent email verification to user
...
def forgot_password(request):
    if request.method == 'POST':
        email = request.POST['email']
        if User.objects.filter(email=email).exists(): # we check if the user with forgot password issue email exist in db
            user = User.objects.get(email__exact=email) #get the exact email from db as entered by user
            #send reset password email
            mail_subject = "Reset Your Password"
            email_template = "accounts/emails/reset_password_email.html"
            send_verification_email(request, user, mail_subject, email_template) #this function exist in utils file
            
            messages.success(request, 'Password reset link has been sent to your email address') # give output when user resets password
            return redirect('login') #take user to login page
        else:
            messages.error(request, 'Account does not exist') # give output when user email is not found
            return redirect('forgot_password') #take user to login page
...
def reset_password_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token): # we validate the user token
        request.session["uid"] = uid #we store the uid inside a session
...
def reset_password(request):
    if request.method == "POST":
        ...
        if password == confirm_password:
            pk = request.session.get("uid") #get primary key of user whose password should be change
            user = User.objects.get(pk=pk) #check where user primary key matches the specified
            user.set_password(password) #we set password to new password
            user.is_active = True
            user.save()
